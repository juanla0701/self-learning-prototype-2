(function (root) {
  ...
  function maybeShowEmpty() {
    const list = document.getElementById("list");
    if (State.symbols.length === 0 && !list.querySelector(".empty")) {
  ...
  function switchView(view) {
    State.view = view;
    document.getElementById("list").style.display = view === "signals" ? "" : "none";
    document.getElementById("recordsView").classList.toggle("open", view === "records");
    document.getElementById("menuSignalsBtn").classList.toggle("active", view === "signals");
  ...
  /* ---------------- 종목 추가 시 선택할 카테고리 (코인/주식 선물) ---------------- */
  let selectedCategory = "coin";
  function getSelectedCategory() { return selectedCategory; }
  function setSelectedCategory(cat) {
    selectedCategory = cat;
    document.querySelectorAll("#categoryPicker button[data-category]").forEach((b) => {
  ...
  function renderChips() {
    ...
    CONFIG.CATEGORIES.forEach((cat) => {
      const syms = State.symbols.filter((s) => State.getCategory(s) === cat);
      if (syms.length === 0) return;
      const group = document.createElement("div");
      group.className = "chip-group";
      const title = document.createElement("div");
      title.className = "chip-group-title";
      title.textContent = t(cat === "coin" ? "coinFutures" : "stockFutures") + ` (${syms.length})`;
      ...
      syms.forEach((sym) => {
        ...
        chip.innerHTML = `
          ...
          <button data-sym="${sym}" aria-label="remove">&times;</button>`;
        group.appendChild(chip);
      });
      box.appendChild(group);
    });
    document.getElementById("symbolCount").textContent = `${State.symbols.length} / ${CONFIG.MAX_SYMBOLS}`;
  }
